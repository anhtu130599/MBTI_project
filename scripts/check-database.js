...
async function checkDatabase() {
  try {
    ...
    // Check total count
    const count = await PersonalityDetailInfo.countDocuments();
    console.log(`Total PersonalityDetailInfo records: ${count}`);

    // List all types
    const types = await PersonalityDetailInfo.find({}, 'type name').sort({ type: 1 });
    ...
    // Check specific ESTJ
    const estj = await PersonalityDetailInfo.findOne({ type: 'ESTJ' });
    if (estj) {
      console.log('\nESTJ found in database:');
      console.log(`- Type: ${estj.type}`);
      console.log(`- Name: ${estj.name}`);
      console.log(`- Description: ${estj.description.substring(0, 100)}...`);
    } else {
      console.log('\nESTJ NOT FOUND in database!');
    }
